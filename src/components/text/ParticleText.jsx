...
/* Texto formado por partículas: o desenho da palavra é amostrado num canvas
 * fora de tela, cada pixel aceso vira uma partícula, e elas se juntam a partir
 * de posições espalhadas. Depois de formadas, ficam à deriva de leve e fogem
 * do cursor.
 *
 * Componente do React Bits, adaptado ao repo: 'use client' no topo (a página é
 * server component) e o CSS movido para o globals.css. A lógica é a original.
 *
...
/* `fontSize` pode vir como clamp(): mede num elemento sonda para virar px, que
   é o que o canvas entende. */
...
const waitForFonts = async (font) => {
  ...
  try {
    ...
    /* fonte indisponível: segue com o fallback */
  }
...
export default function ParticleText({
  ...
  useEffect(() => {
    ...
    const drawParticle = (particle) => {
      ...
      /* partícula pequena vira retângulo: arco custa caro em milhares delas */
      if (size <= 2.1) {
    ...
    /* Amostragem: desenha a palavra num canvas fora de tela e transforma cada
       pixel aceso num alvo de partícula. */
    const sampleText = async () => {
      ...
      /* não coube: reduz o corpo e mede de novo */
      ...
      /* teto de partículas proporcional à área, para não afundar o quadro */
