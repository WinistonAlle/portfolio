...
/* Título que se desenha: primeiro o contorno de cada letra é traçado, depois
 * a cor preenche por baixo. Componente do React Bits, adaptado ao repo:
 * 'use client' no topo (a página é server component), CSS movido para o
 * globals.css junto com o resto dos componentes portados, e o import do
 * ScrollTrigger só acontece quando o trigger é "scroll" (senão o bundle
 * carrega um plugin que a página não usa).
 *
 * Convenção do repo para componente portado: .jsx com tipos à mão no .d.ts
 * ao lado, igual CardSwap, Lanyard e StackGraph.
 */
...
export default function StrokeText({
  ...
  /* mede o bbox real do texto para o viewBox abraçar exatamente as letras */
  useLayoutEffect(() => {
    ...
    measure();
    /* as fontes do next/font chegam depois do primeiro paint e mudam a
       métrica das letras, então mede de novo quando elas carregam */
  ...
  useEffect(() => {
    ...
    /* O componente original começava o preenchimento em drawDuration +
       fillDelay, ignorando o stagger: as últimas letras ainda estavam sendo
       traçadas quando a cor já passava por cima delas. Some com durações
       curtas, escancara com as longas. */
    const drawEnd = drawDuration + stagger * Math.max(0, strokes.length - 1);
    ...
    if (trigger === 'hover') {
      ...
      setStart();
      /* import dinâmico: sem isso o ScrollTrigger entra no bundle mesmo em
         páginas que só usam o trigger "mount" */
