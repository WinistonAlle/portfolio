 ...
 *    alguma prop de interação estiver ligada. Este site já teve um canvas
 *    engolindo o gesto de rolagem no celular; um fundo decorativo não pode nem
 *    chegar perto disso.
 * 2. Teto de DPR configurável (`dprMax`), em 1.5 no uso real em vez de 2. É
 *    fragmento puro em tela cheia: em 2x o custo dobra sem ganho visível num
 ...
 *    original esquecia.
 *
 * A checagem de WebGL2 e o try/catch em volta do renderer são do original e
 * ficam: ver `src/lib/tem-webgl.ts` para o que este site aprendeu sobre
 * bibliotecas que assumem que o contexto sempre abre.
 */
...
export default function MagicRings({
  ...
  /* `interativo` decide se o contêiner chega a escutar o ponteiro. Fora dele,
     o div fica com `pointer-events: none` e nem recebe evento para ignorar. */
  const interativo = followMouse || clickBurst || hoverScale !== 1 || parallax !== 0;
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      /* O React pode já ter tirado o nó: desmontar não pode virar exceção. */
      try {
  ...
  return (
    <div
      ...
      style={{
        ...
        ...(interativo ? null : { pointerEvents: 'none' }),
      }}
    />
  );
}
