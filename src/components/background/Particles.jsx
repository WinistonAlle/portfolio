...
const Particles = ({
  ...
  /** 0 means "match the display", read after mount where window exists. */
  pixelRatio = 0,
  ...
  useEffect(() => {
    ...
    // The field is a fixed, pointer-events:none layer, so the pointer never
    // reaches the container — track the mouse on window instead.
    ...
    // Scroll progress in viewports, read in the render loop so scrolling never
    // triggers a React render.
    const handleScroll = () => {
      scrollRef.current = window.scrollY / Math.max(window.innerHeight, 1);
    };
    const scrollDriven = scrollParallax !== 0 || scrollRoll !== 0;
    if (scrollDriven) {
    ...
    // Someone who asked for less motion still gets the field, just still.
    ...
    const update = (t) => {
      ...
      smoothedScroll += (scrollRef.current - smoothedScroll) * 0.08;
